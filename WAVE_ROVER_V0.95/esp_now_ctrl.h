                ...
                int len)
{
  ...
  // Copy incoming payload into espNowMegsRecv as before
  memcpy(&espNowMegsRecv, incomingData, sizeof(espNowMegsRecv));

  if (espNowMegsRecv.cmd == 3) {
    // Build MAC string from sender (now info->src_addr)
    char macStr[18];
    snprintf(macStr, sizeof(macStr), "%02x:%02x:%02x:%02x:%02x:%02x",
  ...
  // If broadcast-only mode is disabled, enforce whitelist
  if (!ctrlByBroadcast) {
    // compare sender MAC against whitelist
